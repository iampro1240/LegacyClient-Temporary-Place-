    ...
    espConnection = Variables.RunService.PreRender:Connect(function(deltatime)
      ...
      for _, player in espCache do
          ...
          lastTick += deltatime
            ...
            if not Client.Character or not character or not rayOrigin then
              esp.Visible = false
            ...
            local bottom2D, isBottomVisible = WorldToViewportPoint(Camera, rootPos - Variables.Vector3new(0, halfHeight, 0))
                ...
                do -- Box
                    ...
                    if isGradientSpin then
                      if elapsed >= 1 / elapsed * gradientAnimationSpeed then
                       boxFill.UIGradient.Rotation += boxFillRotationSpeed
                      end
                     else
...
function ESP:loadESP()
  local clientCharacter = Variables.LocalPlayer.Character
  ...
  rayOrigin = clientCharacter.Head or WaitForChild(clientCharacter, "Head")
...
return library, themes, ESP, espConnection
